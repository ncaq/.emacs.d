...
(leaf
 ...
 :ensure t
 :preface
 ;; `prettier-rc-mode'が有効なバッファでは`lsp-mode'の保存時フォーマットを抑止します。
 (defun prettier-rc--suppress-lsp-format-on-save (orig-fun &rest args)
   "`prettier-rc-mode'有効時は`lsp--format-buffer-before-save'を実行しない。"
   (unless (bound-and-true-p prettier-rc-mode)
     (apply orig-fun args)))
 ;; 関数定義レベルのアドバイスなので、
 ;; `.dir-locals.el'等で`lsp-format-buffer-on-save'を`t'に上書きされても勝てます。
 :advice (:around lsp--format-buffer-before-save prettier-rc--suppress-lsp-format-on-save)
 :init
 (defun prettier-toggle-setup ()
   ...
   ;; lsp-modeによるフォーマッターよりprettierを優先します。
   (local-set-key [remap lsp-format-buffer] 'prettier-rc)
