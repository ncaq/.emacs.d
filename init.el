  ...
  (mapc
   (lambda (kc)
     (when (or (cdr kc) (lookup-key key-map (car kc)))
       ;; 無駄に`nil'を増やさないように、
       ;; 新規コマンドが`nil'で挿入先キーマップのコマンドも`nil'ならキーを挿入しない。
       (define-key key-map (car kc) (cdr kc))))
   (mapcar
    (lambda (kp) (trans-bind key-map kp)) (append key-pairs (mapcar 'reverse-cons key-pairs)))))
...
(defun dvorak-set-key (key-map)
  ...
  (let ((prefixes
    ...
    (mapc
     (lambda (pp)
       (swap-set-key
        key-map (mapcar (lambda (kp) (prefix-key-pair (car pp) (cdr pp) kp)) qwerty-dvorak)))
     prefixes)))
...
(leaf
 ...
 :bind
 (:isearch-mode-map
  ("C-b" . isearch-delete-char) ("M-b" . isearc-del-char) ("M-m" . isearch-exit-previous))
...
(leaf
 ...
 :config (dvorak-set-key-prog rectangle-mark-mode-map))
 ...
 *global-set-key
 ...
 ("C-o" . helm-for-files-prefer-near)
 ...
 ("M-m" . newline-under)
 ("M-n" . scroll-up-one)
 ("M-o" . helm-for-files-prefer-recentf)
...
(leaf
 tramp
 :custom
 ;; バックアップファイルをroot絡みでも自動許可する。
 (tramp-allow-unsafe-temporary-files . t))

(leaf
 filelock
 :custom
 ;; ロックファイルを作るとディレクトリを監視するツールが誤動作することがあるため、
 ;; ロックファイルを作らないようにします。
...
(leaf
 ...
 (modus-themes-headings
  . '((1 . (rainbow bold 1.3)) (2 . (rainbow bold 1.2)) (t . (rainbow bold 1.1))))
...
(leaf
 ...
 (frame-title-format
  . '(:eval (list "emacs " (or (buffer-file-name) (buffer-name)) " " mode-line-modes)))
 ...
 *c-source-code
 ...
 (read-buffer-completion-ignore-case . t) ; バッファ名の大文字と小文字を区別しない
 (read-file-name-completion-ignore-case . t) ; ファイル名の大文字と小文字を区別しない
 (read-process-output-max . 3145728) ; プロセスから一度に3MBほどまで読み込む量を増やす
 (ring-bell-function . #'ignore) ; ビープ音を消す
 (scroll-conservatively . 1) ; 最下段までスクロールした時のカーソルの移動量を減らす
 (scroll-margin . 5) ; 最下段までスクロールしたという判定を伸ばす
 `(user-full-name . ,(user-login-name))) ; `NAME'が`hostname'と同じ値になってしまう環境への対応

(leaf autorevert :doc "自動再読込。" :global-minor-mode global-auto-revert-mode)
...
(leaf
 executable
 :doc "スクリプトに実行権限を自動で付加。"
 :hook (after-save-hook . executable-make-buffer-file-executable-if-script-p))

(leaf
 files
 :custom
 ;; ファイルを保存する時に自動的に最後に改行を入れます。
 ;; POSIXのテキストファイルの規定を守ります。
 (require-final-newline . t))

(leaf indent :doc "標準インデント値を設定。" :custom (standard-indent . 2))

(leaf
 simple
 :custom
 (blink-matching-paren . nil) ; 括弧移動無効
 (kill-ring-max . 600)) ; メモリに余裕があるのでクリップボードの履歴数を増やします

(leaf
 smie
 :config
 ;; SMIEを使うモードのインデントをファイルごとに編集されても良いようにします。
 (add-to-list 'safe-local-variable-values '(smie-indent-basic . 2)))

(leaf
 novice
 :custom
 ;; 初心者向けに無効設定がされたコマンドの起動時の警告をなくすことで実質無効化されなくします。
 ...
 *wl-clipboard
 ...
 :when (featurep 'pgtk)
 :init
 ;; Wayland向けにwl-clipboard連携でシステムとクリップボードを共有します。
 ...
 :custom
 (interprogram-cut-function . 'wl-copy) ; cutをwl-copyに任せる
 (interprogram-paste-function . 'wl-paste)) ; pasteをwl-pasteに任せる
...
(leaf
 ...
 (defun helm-for-files-prefer-recentf ()
   ...
   (let ((helm-for-files-preferred-list
     ...
     (helm
      :sources helm-for-files-preferred-list
 ...
 (leaf
  ...
  (setq auto-mode-alist
        (cl-remove-if
         (lambda (pair)
           (member (cdr pair) '(helm-ls-git-commit-mode helm-ls-git-rebase-todo-mode)))
         auto-mode-alist))
...
(leaf
 ...
 :custom
 (browse-url-generic-program . "wslview") ; WSL向けのブラウザ起動コマンド
 (browse-url-browser-function . 'browse-url-generic) ; URLを開く挙動のデフォルトを指定
 :config
 ;; xdg-openのデスクトップ環境の特定挙動にはWayland環境かどうかが影響するので、
 ;; WSLではxorg環境と違ってWaylandではWindows側のコマンドを開いてくれません。
 ;; 明示的に環境変数に設定することでxdg-openの最後のフォールバックとして確定させて、
 ;; Wayland環境でもEmacs内部からxdg-openに依存したブラウザ起動を利用できようにします。
 (setenv "BROWSER" browse-url-generic-program))
...
(leaf
 ...
 (company-idle-delay . nil) ; companyの自動補完は少し鬱陶しいので無効化します。
 :bind
...
(leaf
 ...
 :bind
 (:yas-minor-mode-map
  ("<tab>" . nil) ("TAB" . nil) ("C-c C-y" . company-yasnippet) ("M-z" . yas-insert-snippet))
...
(leaf
 ...
 (defun turn-on-copilot-mode ()
   (interactive)
   (copilot-mode 1))
 (defun remove-copilot-shutdown-from-kill-emacs (&rest _)
   "`copilot--start-server'は、
サーバ起動のたびに`copilot--shutdown-server'を`kill-emacs-hook'へ登録します。
`copilot--shutdown-server'はnode言語サーバへJSON-RPCの`shutdown'を送って応答を待ちます。
しかしsystemdによるデーモンモードを使っていて、
`KillMode=control-group'で子プロセスもemacs本体と同時にSIGTERMされる状況では、
既に終了したnode言語サーバから応答が返ってこないため応答待ちで固まります。
結果としてサービス停止がsystmedの`TimeoutStopSec'(既定で約90秒)まで、
システムのシャットダウン全体が遅延します。
emacsサービス終了時に子プロセスはsystemdによりどのみち回収されるため、
デーモン使用時は`kill-emacs-hook'から起動直後に`copilot--shutdown-server'を除去して、
終了をブロックしないようにします。
https://github.com/copilot-emacs/copilot.el/issues/469"
   (when (daemonp)
     (remove-hook 'kill-emacs-hook #'copilot--shutdown-server)))
 ;; 起動時に無限再帰してしまう問題から`global-copilot-mode'をしばらく諦める。
 ;; issueはcloseしていますが問題は継続していることを確認しています。
 ;; <https://github.com/copilot-emacs/copilot.el/issues/226>
 :hook ((conf-mode-hook prog-mode-hook text-mode-hook) . turn-on-copilot-mode)
 :advice (:after copilot--start-server remove-copilot-shutdown-from-kill-emacs)
...
(leaf
 ...
 :ensure t
 :custom
 (copilot-chat-default-model . "gpt-5.4")
 ...
 ("C-; C-c" . copilot-chat-regenerate-commit-message)
 ...
 :hook (git-commit-setup-hook . copilot-chat-insert-commit-message-no-clobber))
...
(leaf
 ...
 (defun string-inflection-dwim-style-cycle ()
   "メジャーモードに従ってありえる命名候補を自動的に変更します。
判断できなければ`string-inflection-all-cycle'に投げます。"
   (interactive)
   (pcase major-mode
     ;; snake_case => UPPER_SNAKE_CASE => PascalCase
     ((or 'c++-mode 'c-mode 'ruby-mode 'rust-mode 'rustic-mode 'tuareg-mode)
      (string-inflection-ruby-style-cycle))
     ;; snake_case => PascalCase
     ('elixir-mode (string-inflection-elixir-style-cycle))
     ;; snake_case => UPPER_SNAKE_CASE => PascalCase
     ('python-mode (string-inflection-python-style-cycle))
     ;; camelCase => UPPER_SNAKE_CASE => PascalCase
     ((or 'csharp-mode
          'd-mode
          'elm-mode
          'groovy-mode
...
(leaf
 ...
 :defun prettier-rc
 :preface
 (defun prettier-rc-override-other-format (orig-fun &rest args)
   "`prettier-rc-mode'が有効な時は`ORIG-FUN'の代わりに`prettier-rc'を呼び出します。
`prettier-rc'はアドバイスの引数は無視します。"
   (if (bound-and-true-p prettier-rc-mode)
       (prettier-rc)
     (apply orig-fun args)))
 ;; `prettier-rc-override-other-format'を`lsp-format-buffer'のアドバイスとして登録します。
 ;; prettierが対応している言語のフォーマッターがだいたいprettierより安定せず、
 ;; prettierに比べてコードを壊してしまうことが多いので、
 ;; 置き換えると有用です。
 ;; 関数定義レベルのアドバイスなので、
 ;; `.dir-locals.el'等で`lsp-format-buffer-on-save'を`t'に上書きされても勝てます。
 :advice (:around lsp-format-buffer prettier-rc-override-other-format)
 :init
 (defun prettier-toggle-setup ()
   "prettierの有効無効キーバインドを使えるようにして、自動prettier適応を有効にします。"
   (interactive)
   ;; 全体フォーマットをEmacsではなくprettierが行うようにします。
   (local-set-key [remap indent-whole-buffer] 'prettier-rc)
   ;; M-iでprettier-rcの一時的無効化が出来るようにします。
...
(leaf sops :ensure t :global-minor-mode global-sops-mode)
...
大抵はmasterかmain。"
     (let ((remote (magit-primary-remote)))
       (magit-git-string "symbolic-ref" "--short" (format "refs/remotes/%s/HEAD" remote)))))
 (defun magit-find-file-to-default-branch ()
   "今いるファイルのデフォルトブランチのリビジョンを開く。"
   (interactive)
   (magit-find-file (magit-get-default-branch) (buffer-file-name)))
 (defun magit-find-file-to-head ()
   "今いるファイルのHEADのリビジョンを開く。"
   (interactive)
   (magit-find-file "HEAD" (buffer-file-name)))
 (defun magit-diff-range-to-default-branch ()
   "今いるファイルのデフォルトブランチとのdiffを閲覧する。"
   (interactive)
   (magit-diff-range (magit-get-default-branch) nil (list (buffer-file-name))))
...
Forgeとかにも作成機能はあるが、レビュアーやラベルやProjectsの指定はwebの方が楽。"
   (interactive)
   (let ((process (magit-run-git-async "push" "--verbose" "--set-upstream" "origin")))
 ...
 ("M-g M" . magit-diff-range-to-default-branch)
 ...
 ("M-g m" . magit-find-file-to-default-branch)
 ...
 (leaf
  ...
  ;; `magit-diff-visit-worktree-file'は`C-<return>'でも代用出来て、
  ;; 検索の誤爆の原因になるので無効化する。
...
(leaf
 ...
 (lsp-disabled-clients . '(copilot-ls)) ; 専用のcompanyベースのcopilot.elの方が高機能なため無効化
 ...
 (leaf
  ...
  (lsp-ui-sideline-enable . nil) ; エラーはflycheckで出すので幅を取るサイドラインは不要
  :bind
  (:lsp-ui-mode-map
...
(leaf
 ...
 (leaf
  ...
  :bind
  (:d-mode-map
   ([remap indent-whole-buffer] . dfmt-buffer) ([remap save-buffer] . dfmt-save-buffer)))
 (leaf
  company-dcd
  :ensure t
 ...
 (eval-and-compile
   ...
   (defun elisp-autofmt-check-elisp-autofmt-exists-and-not-package ()
     (and (elisp-autofmt-check-elisp-autofmt-exists)
          (not (in-user-emacs-directory-not-user-config-p))))
...
(leaf
 ...
 ;; ReactなどのJSXを含むTypeScript/JavaScriptファイル。
 ;; 現実的には`.ctsx'などはあまりないと思いますが一応マッピングします。
...
(leaf
 ...
 :after t
 :custom
 ;; このlspのフォーマットはコードを壊しやすいので無効化します。
 ;; prettierに任せます。
 (lsp-javascript-format-enable . nil)
 ;; 同じパッケージで提供されているTypeScript向けも同様です。
...
面倒なので通常の`json-mode'でまとめて扱います。"
 ...
 (leaf
  ...
  (setq lsp-language-id-configuration
        (cl-remove-if
         (lambda (entry)
           (and (stringp (car entry)) (string-equal (car entry) "\\.json$")))
...
(leaf
 ...
 :config
 (add-to-list 'safe-local-variable-values '(rustic-indent-offset . 4))
